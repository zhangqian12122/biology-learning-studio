    ...
    <div className="rounded-lg border border-[#cfe0e0] bg-[#fbfdfd] p-4">
        ...
        {specimen.parts.map((part, index) => (
          <button
            ...
            className={`inline-flex min-h-9 items-center gap-1.5 rounded-md border px-2.5 text-xs font-semibold transition-colors ${
              activePart === index
                ? 'border-[#82c6c0] bg-[#e9f7f5] text-[#1d7fa8]'
                : 'border-[#d9e7e7] bg-white text-gray-600 hover:border-[#b6d9d6]'
            }`}
          >
      ...
      {selectedPart ? (
        <div className="mt-3 rounded-md border border-[#cfe1e0] bg-[#eef7f6] px-3 py-2.5">
          <p className="text-xs font-bold text-[#1d7fa8]">{selectedPart.name}</p>
          <p className="mt-1 text-xs leading-5 text-gray-600">{selectedPart.desc}</p>
        </div>
      ) : (
        <p className="mt-3 text-xs text-gray-400">点选任一结构名，图中会高亮对应部分并显示考点说明。</p>
      )}
    </div>
  );
}
