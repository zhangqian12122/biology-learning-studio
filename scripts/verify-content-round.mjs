...
await page.waitForTimeout(2500);
{
  ...
  results.experiment.open = before.includes('富营养化与水华');
  results.experiment.interactive = after.includes('死亡谷') && after.includes('水华');
  results.experiment.reference = after.includes('注意事项');
  results.experiment.dayNight = before.length > 0;
}

// ---------- 标本：深链直达 + SVG 文字几何检查 ----------
const SPECIMENS = ['honeyBadger', 'melanin', 'airPlant'];
...
// 回归检查：ecoStability 实验页仍正常
await page.goto(`${BASE}/lab?exp=ecoStability`, { waitUntil: 'domcontentloaded' });
await page.waitForTimeout(1800);
{
  const t = await page.locator('main').innerText();
  results.specimens.ecoRegression = { found: t.includes('生态系统') || t.includes('物种'), ok: t.includes('生态系统') || t.includes('物种') };
}
...
const allOk = expOk && SPECIMENS.every((id) => results.specimens[id]?.ok) && results.specimens.ecoRegression?.ok;
